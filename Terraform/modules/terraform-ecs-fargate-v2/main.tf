...
resource "aws_iam_role_policy" "sqs" {
  name   = "${var.name_prefix}-ecs-sqs-permissions"
  role   = aws_iam_role.task.id
  policy = data.aws_iam_policy_document.task_ecs_sqs_policy.json
}

resource "aws_iam_role_policy" "sns" {
  name   = "${var.name_prefix}-ecs-sns-permissions"
  role   = aws_iam_role.task.id
  policy = data.aws_iam_policy_document.task_ecs_sns_policy.json
}
...
[{
  ...
  %{~endif}
  %{if local.ecs_secrets != null~}
  "secrets": ${jsonencode(local.ecs_secrets)},
  %{~endif}
